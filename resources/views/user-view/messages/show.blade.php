        ...
        <div class="middle-sidebar-bottom">
            <div class="middle-sidebar-left pe-0">
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <p><strong>{{ __("messages.From:") }}</strong> {{ $message->sender ? $message->sender->name : 'Admin' }}</p>
                        <p><strong>{{ __("messages.To:") }}</strong> {{ $message->recipient ? $message->recipient->name : 'User' }}</p>
                        <hr>
                        ...
                        <div class="card-body">
                            @foreach ($message->replies as $reply)
                                <div class="d-flex mb-4 {{ $reply->sender_id == 0 ? 'justify-content-end' : '' }}">
                                    <div class="p-3 rounded shadow-sm {{ $reply->sender_id == 0 ? 'bg-primary text-white text-end' : 'bg-light text-dark' }}"
                                        style="max-width: 75%;">
                                        <p class="mb-1">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('user.messages.reply', $message->id) }}">
                            ...
                            <button type="submit" class="btn btn-primary"> {{ __("messages.Send Reply") }}</button>
                        </form>
                    </div>
                </div>
            </div>
